...
const TimeAnswerComponent = (props: any) => {
  return (
    <NodeViewWrapper className="time-answer-block" data-required={props.node.attrs.required ? "true" : undefined}>
      <div className="question-title-row">
        <NodeViewContent 
          as="div"
          className="time-answer-title outline-none" 
        />
...
export const TimeAnswerBlock = Node.create({
  name: "timeAnswerBlock",
  group: "block",
  content: "inline*",
  draggable: true,
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="time-answer-block"]' }];
  },

  renderHTML({ HTMLAttributes }) {
    return ["div", mergeAttributes(HTMLAttributes, { "data-type": "time-answer-block" }), 0];
  },

  addNodeView() {
    return ReactNodeViewRenderer(TimeAnswerComponent);
  },
  ...
  addKeyboardShortcuts() {
    return {
      Enter: () => {
        ...
        if (!empty || $from.parent.type.name !== "timeAnswerBlock") return false;
