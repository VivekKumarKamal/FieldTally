...
const PhoneAnswerComponent = (props: any) => {
  return (
    <NodeViewWrapper className="phone-answer-block" data-required={props.node.attrs.required ? "true" : undefined}>
      <div className="question-title-row">
        <NodeViewContent 
          as="div"
          className="phone-answer-title outline-none" 
        />
...
export const PhoneAnswerBlock = Node.create({
  name: "phoneAnswerBlock",
  group: "block",
  content: "inline*",
  draggable: true,
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="phone-answer-block"]' }];
  },

  renderHTML({ HTMLAttributes }) {
    return ["div", mergeAttributes(HTMLAttributes, { "data-type": "phone-answer-block" }), 0];
  },

  addNodeView() {
    return ReactNodeViewRenderer(PhoneAnswerComponent);
  },
  ...
  addKeyboardShortcuts() {
    return {
      Enter: () => {
        ...
        if (!empty || $from.parent.type.name !== "phoneAnswerBlock") return false;
