...
const EmailAnswerComponent = (props: any) => {
  return (
    <NodeViewWrapper className="email-answer-block" data-required={props.node.attrs.required ? "true" : undefined}>
      <div className="question-title-row">
        <NodeViewContent 
          as="div"
          className="email-answer-title outline-none" 
        />
...
export const EmailAnswerBlock = Node.create({
  ...
  parseHTML() {
    return [{ tag: 'div[data-type="email-answer-block"]' }];
  },

  renderHTML({ HTMLAttributes }) {
    return ["div", mergeAttributes(HTMLAttributes, { "data-type": "email-answer-block" }), 0];
  },

  addNodeView() {
    return ReactNodeViewRenderer(EmailAnswerComponent);
  },
  ...
  addKeyboardShortcuts() {
    return {
      Enter: () => {
        ...
        if (!empty || $from.parent.type.name !== "emailAnswerBlock") return false;
